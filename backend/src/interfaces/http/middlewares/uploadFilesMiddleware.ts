...
/** Limit size of upload files and validate file types */
export const uploadFilesMiddleware = (
  ...
  upload.array("files")(req, res, (err) => {
        ...
        switch (err.code) {
          case "LIMIT_FILE_SIZE":
            return res
              ...
              .json({
                error:
                  "A file exceeds the maximum allowed size of 20MB per file",
              });
          case "LIMIT_FILE_COUNT":
            return res
              .status(400)
              .json({ error: "Too many files. Maximum allowed is 50" });
          default:
            return res
              .status(500)
    ...
    if (totalSize > MAX_TOTAL_SIZE_PER_REQUEST) {
      return res.status(400).json({
        error: `Total file size is ${(totalSize / (1024 * 1024)).toFixed(
          2
        )}MB, which exceeds the limit allowed of ${(
